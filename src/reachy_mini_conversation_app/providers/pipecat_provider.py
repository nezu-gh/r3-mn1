...
# Default is lean — augmentations are opt-in via PIPELINE_MODE=full.
PIPELINE_MODE = os.environ.get("PIPELINE_MODE", "lean").lower()
...
class _PipelineMetricsObserver:
    ...
    async def cleanup(self) -> None:
        """Lifecycle hook required by pipecat >=1.0 (no-op)."""
        pass
    ...
    async def start_up(self) -> None:
        ...
        llm = OpenAILLMService(
            ...
            settings=OpenAILLMService.Settings(
                ...
                # Must use extra_body so the OpenAI SDK includes it in the
                # request body.  Pipecat merges `extra` into create() kwargs,
                # and extra_body is the SDK's mechanism for arbitrary body fields.
                extra={
        ...
        # Pipecat's default stop strategy is the SmartTurn ONNX analyzer, which
        # waits up to several seconds after VAD-stop to decide the user is done.
        # Swap it for a simpler speech-timeout (ends the turn N seconds after
        # VAD-stop) — shaves 0.5–3s off the first-audio latency.
        _user_speech_timeout = float(os.environ.get("USER_SPEECH_TIMEOUT", "0.6"))
        user_params = LLMUserAggregatorParams(
            user_turn_strategies=UserTurnStrategies(
                start=[MinWordsUserTurnStartStrategy(min_words=_min_interrupt_words)],
                stop=[SpeechTimeoutUserTurnStopStrategy(user_speech_timeout=_user_speech_timeout)],
            ),
        )
        ...
        logger.info(
            "MinWords interruption: %d words to interrupt; speech-timeout stop: %.2fs",
            _min_interrupt_words, _user_speech_timeout,
        )
